...
class TestChunkedgraph():
    ...
    def test_past_ids(self, myclient):
        ...
        id_map={'future_id_map': {},
                'past_id_map': {864691135415612346: [864691134989972295, 864691135574118596],
        ...
        qid_map=myclient.chunkedgraph.get_past_ids(root_ids,
                                                   timestamp_past=past_time,
                                                   timestamp_future=now)
        assert(qid_map == id_map)
        qid_map=myclient.chunkedgraph.get_past_ids(root_id_list,
                                                   timestamp_past=past_time,
                                                   timestamp_future=now)
        assert(qid_map == id_map)
    ...
    def test_lineage_graph(self, myclient):
        ...
        endpoint_mapping["root_id"] = root_id
        url = chunkedgraph_endpoints_v1['handle_lineage_graph'].format_map(endpoint_mapping)
        now = datetime.datetime.utcnow()
        past_time = now - datetime.timedelta(days=7)
        query_d ={
            'timestamp_past':time.mktime(past_time.timetuple()),
        ...
        lineage_graph = {'directed': True,
                        'graph': {},
                        'links': [{'source': 864691136089107255, 'target': 864691135490360423},
                        {'source': 864691135348456151, 'target': 864691136089107255}],
                        'multigraph': False,
                        'nodes': [{'id': 864691136089107255,
                        'operation_id': 225368,
        ...
        qlineage_graph = myclient.chunkedgraph.get_lineage_graph(root_id,
                                                                 timestamp_past=past_time,
                                                                 timestamp_future=now)
    ...
    def test_operatin_details(self, myclient):
        endpoint_mapping = self._default_endpoint_map
        url = chunkedgraph_endpoints_v1['operation_details'].format_map(endpoint_mapping)
        ...
        operation_details = {'217696': {'bb_offset': [240, 240, 24],
                            'operation_exception': '',
                            'operation_status': 0,
                            'operation_ts': '2021-03-25 19:06:18.177000+00:00',
                            'removed_edges': [[99450140208950705, 99450140208951689],
                            ...
                            'user': '121'},
                            '225368': {'added_edges': [[99093279832296446, 99093279832302829]],
                            'operation_exception': '',
                            'operation_status': 0,
        ...
        qoperation_details = myclient.chunkedgraph.get_operation_details(operation_id_list)
        assert(operation_details==qoperation_details)
    ...
    def test_get_info(self, myclient):
        ...
        test_info={'app': {'supported_api_versions': [0, 1]},
